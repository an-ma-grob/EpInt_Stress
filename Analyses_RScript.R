...
#V = "RSA_RightAntHC_AB_pre"
V = "Link_AB"
total[total$Stress.x== G,][which(total[total$Stress.x== G, V] > (mean(pull(total[total$Stress.x== G, V]), na.rm = T) + (2.5*SD(total[total$Stress.x== G, V])))),]
total[total$Stress.x== G,][which(total[total$Stress.x== G, V] < (mean(pull(total[total$Stress.x== G, V]), na.rm = T) - (2.5*SD(total[total$Stress.x== G, V])))),]
...
d_wide <- my_data[,c(1,2,4,48,49,50,51,52)]
...
d_wide <- d_wide %>%
  mutate(AUCi = AUCg - Cort_1 * (25 + 40 + 20 +30))

my_data$AUCg <- d_wide$AUCg
my_data$AUCi <- d_wide$AUCi
...
my_data$Post_Diff <- my_data$Mean_LinkPost - my_data$Mean_NonLinkPost
...
# ARENA NonLink
my_data <- backup_2
my_data <- my_data[c(-20, -10, -16),] #AUCi
V = "AUCi"
...
ggplot(my_data[my_data$Stress.x == G ,], aes_string(V, C)) +
  geom_point(size=3, shape=19, colour = "brown1") + apatheme  + geom_smooth(method='lm', colour = "brown1", fill = "brown1", alpha = 0.2) +
...
# R ant HC post AB 
my_data <- backup_2
my_data <- my_data[c(-20, -10, -16, -38),] #AUCi
V = "AUCi"
C = "RSA_RightAntHC_AB_pre"
...
Target <- total 
Target <- Target[c(-47, -44),]

t.test(total$Target[total$Stress.x == "Control"],
       total$Target[total$Stress.x == "Stress"],paired = F)
...
Integration <- Integration[c(-19, -2, -1, -8),]
...
p <-ggplot(Integration_long[Integration_long$Condition=="Presentation" & Integration_long$Stress=="Stress",], aes(x = Link, y = Integration, fill = Time))+
  ...
  geom_point(aes(colour=Time),pch = 19, size = 2, position = position_dodge(0.6), # new version (no more jittering)
             alpha = 0.4)+
  ...
  ylab('Link rating') + xlab ('') + stat_summary(fun.data = mean_se, size = 1, geom = "errorbar",  position=position_dodge(0.6),width =.1)+
  apatheme

p+ coord_cartesian( ylim = c(1, 4.5))+ ggtitle("Stress Group") +
  annotate(geom="text", x=1, y=4.3, label= c("* * *"), color="black", fontface = "bold", size = 7) +
  geom_segment(aes(x=0.9,xend=1.1,y=4.2,yend=4.2)) +
  annotate(geom="text", x=2, y=3.5, label= c("* * *"), color="black", fontface = "bold", size = 7) +
...
p <-ggplot(Cued[Cued$Condition.x=="Presentation",], aes(x = Group, y = CuedRecall*100, fill = Group))+
  ...
  geom_point(aes(x = x, colour=Group, fill = Group),pch = 21, size = 2, position = position_jitterdodge(jitter.width = 0.05, dodge.width = 0.4), alpha = 0.6)+
  ...
  apatheme + ylab('Correct responses (%)') + stat_summary(aes(x = x),fun.data = mean_se, size = 1, geom = "errorbar",  position=position_dodge(0.9),width =.05)+
  scale_x_continuous(expand=expansion(mult=c(0.2,1)), breaks = c(1,1.5,2,2.5), labels = c('Control','Stress','',''))

p+ coord_cartesian(ylim = c(10, 115)) + theme(legend.position = c(0.7,0.9), axis.ticks = element_blank())+
  scale_y_continuous(breaks=c(20, 40, 60, 80, 100)) +
  annotate(geom="text", x=1.25, y=115, label= c("*"), color="black", fontface = "bold", size = 7) +
  geom_segment(aes(x=1,xend=1.5,y=110,yend=110))
...
p <-ggplot(Cued[Cued$Condition.x=="Presentation",], aes(x = Group, y = Cued_Recall_Mean_Certainty, fill = Group))+
  stat_summary(aes(x = x),fun='mean',geom='bar', position=position_dodge(0.9), width = 0.4, alpha = 0.6)+
  geom_point(aes(x = x, colour=Group, fill = Group),pch = 21, size = 2, position = position_jitterdodge(jitter.width = 0.02, dodge.width = 0.4), alpha = 0.6)+
  scale_fill_manual("Group",values = c("Control" = "darkslategray4","Stress"="brown1")) + 
  scale_colour_manual(values=c("darkslategray4", "brown1"))+ xlab('')+
  apatheme + ylab('Certainty rating') + stat_summary(aes(x = x),fun.data = mean_se, size =1, geom = "errorbar",  position=position_dodge(0.9),width =.05)+
  scale_x_continuous(expand=expansion(mult=c(0.2,1)), breaks = c(1,1.5,2,2.5), labels = c('Control','Stress','',''))

p+ coord_cartesian(ylim = c(0.7, 4.3)) + theme(legend.position = c(0.7,0.9), axis.ticks = element_blank())
...
cor.test(Cor1$Mean_LinkPost, 
         Cor1$CuedRecall)


Cor1 <- total
Cor1 <- Cor1[c(-44,-19, -2, -1, -8),]
Cor1$Insight_Diff <- Cor1$Mean_LinkPost - Cor1$Mean_NonLinkPost
...
p <-ggplot(ARENA_long[ARENA_long$Condition=="Presentation" ,], aes(x = Link, y = V, fill = Stress))+
  stat_summary(stat = 'identity', fun='mean',geom='bar', position=position_dodge(0.6), size = 1.5,width = 0.6, alpha = 0.6) +
  geom_point(aes(colour=Stress),pch = 19, size = 2, position = position_dodge(0.6), # new version (no more jittering)
             alpha = 0.4)+
  ...
  ylab('Euclidian distance') + xlab ('') + stat_summary(fun.data = mean_se, size = 1, geom = "errorbar",  position=position_dodge(0.6),width =.1)+
  apatheme 


p+coord_cartesian(ylim = c(.006, .11)) +
  annotate(geom="text", x=1.65, y=.10, label= c("* * *"), color="black", fontface = "bold", size = 7) +
  geom_segment(aes(x=1.15,xend=2.15,y=.098,yend=.098))+
  annotate(geom="text", x=1.35, y=.107, label= c("* * *"), color="black", fontface = "bold", size = 7) +
...
my_data$Link_AB <- (my_data$FreeRecall_A_total.y + my_data$Link_Free)/2
Free <- as.data.frame(my_data[,c(1,2,4,5,6,138,60)])
Free <- Free[c(-8, -11, -16),]
Free_long <-  melt(Free, id.vars = c("VP.x","Code","Stress.x","Sex.x","Age.x",
                                     "Link_AB","NonLink_Free"),measure = names(Free[,c(6,7)]))
names(Free_long) <- c("VP", "Code",  "Stress", "Sex","Age","Link", "NonLink","Item","Details")
...
t.test(Free$Link_AB[Free$Stress.x == "Control"],
       Free$NonLink_Free[Free$Stress.x == "Control"],paired = T)

t.test(Free$Link_AB[Free$Stress.x == "Stress"],
       Free$NonLink_Free[Free$Stress.x == "Stress"],paired = T)


#-------------
# Plot Free Recall 

p <-ggplot(Free_long, aes(x = Item, y = Details, fill = Stress))+
  stat_summary(stat = 'identity', fun='mean',geom='bar', position=position_dodge(0.6), size = 1.5,width = 0.6, alpha = 0.6) +
  geom_point(aes(colour=Stress),pch = 19, size = 2, position = position_dodge(0.6), # new version (no more jittering)
             alpha = 0.4)+
  #geom_line(aes(x = x, group = interaction(Code, Stress), color = Group), alpha = 0.2) +
  ...
  scale_colour_manual("Stress", values = c("Control"="darkslategray4", "Stress" = "brown1")) + 
  #scale_colour_manual("Stress",values = c("Control"="blueviolet", "Stress"="brown4")) +
  ylab('Details') + xlab ('') + stat_summary(fun.data = mean_se, size = 1, geom = "errorbar",  position=position_dodge(0.6),width =.1)+
  apatheme 

Lab <- c("Link", "NonLink")

p+coord_cartesian(ylim = c(0, 29)) + scale_x_discrete(labels= Lab) +
  annotate(geom="text", x=1.65, y=27, label= c("* * *"), color="black", fontface = "bold", size = 7) +
  geom_segment(aes(x=1.15,xend=2.15,y=26,yend=26))+
  annotate(geom="text", x=1.35, y=24, label= c("* * *"), color="black", fontface = "bold", size = 7) +
  geom_segment(aes(x=0.85,xend=1.85,y=23,yend=23))+ theme(legend.position = "none")
...
## Follow up t-tests
t.test(Amygdala[which(Amygdala$Link == "AB" & Amygdala$Group == "Control Pre"),]$V,
       Amygdala[which(Amygdala$Link == "AB" & Amygdala$Group == "Control Post"),]$V, paired = T)
...
p <-ggplot(Amygdala[Amygdala$Stress=="Control",], aes(x = Link, y = V, fill = Time))+
  ...
  scale_fill_manual("Time",values = c("pre"="darkslategray3", "post" = "darkslategray4")) +
  scale_colour_manual(values=c("darkslategray3", "darkslategray4")) + 
  xlab ('') + stat_summary(fun.data = mean_se, size = 1, geom = "errorbar",  position=position_dodge(0.6),width =.1)+
...
p+  ggtitle("aHC: Control Group") + coord_cartesian(ylim = c(0.59, 1))+
  annotate(geom="text", x=1, y=0.95, label= c("*"), color="black", fontface = "bold", size = 7) +
...
# Only remove one person w/ too low value
total <- backup
my_data <- backup
my_data <- my_data[c(-45, -38, -22),]
total <- total[c(-45, -38, -22),]
...
confint(M1, '(Intercept)', level=0.95)
confint(M1, 'RSA_RightAntHC_AB_post', level=0.95)
confint(M1, 'I(RSA_RightAntHC_AB_post^2)', level=0.95)
confint(M1, 'Stress.xStress', level=0.95)


confint(M2, '(Intercept)', level=0.95)
confint(M2, 'RSA_RightAntHC_AB_post', level=0.95)
confint(M2, 'I(RSA_RightAntHC_AB_post^2)', level=0.95)
confint(M2, 'Stress.xStress', level=0.95)
confint(M2, 'RSA_RightAntHC_AB_post:Stress.xStress', level=0.95)
confint(M2, 'I(RSA_RightAntHC_AB_post^2):Stress.xStress', level=0.95)
...
ggplot(prd, aes(x = RSA_RightAntHC_AB_post, y = fit)) +
  ...
  geom_point(data = total[total$Condition.x=="Presentation"&total$Stress.x=="Control",], 
             aes(x = RSA_RightAntHC_AB_post, y = Mean_ARENA_Link))+ coord_cartesian(ylim = c(0, .09), xlim = c(.60, .90))+
...
ggplot(prd, aes(x = RSA_RightAntHC_AB_post, y = fit)) +
  ...
  ylab("Euclidian distance")+ coord_cartesian(ylim = c(0, .09), xlim = c(.60, .90))+
  apatheme +
  geom_line() +
...
# w/ Factor group
Link<-aov_car(V ~ Stress*Time*Link + Error(VP/Time*Link),
              data = Amygdala[Amygdala$Side == "Right",]) #
knitr::kable(nice(Link))


# For Stress right posterior
Link<-aov_car(V ~ Time*Link + Error(VP/Time*Link),
              data = Amygdala[Amygdala$Stress =="Stress" & Amygdala$Side == "Right",]) #
knitr::kable(nice(Link))
...
p+  ggtitle("R pHC: Control Group") + coord_cartesian(ylim = c(0.59, 1)) +
  scale_x_discrete(labels=c("Link", "NonLink"))
...
p <-ggplot(Amygdala[Amygdala$Stress=="Stress"& Amygdala$Side == "Right",], aes(x = Link, y = V, fill = Time))+
  ...
  scale_fill_manual("Time",values = c("pre"="brown1", "post" = "brown4")) +
  scale_colour_manual(values=c("brown1", "brown4")) + 
  xlab ('') + stat_summary(fun.data = mean_se, geom = "errorbar", size = 1,  position=position_dodge(0.6),width =.1)+
  apatheme +labs(y=expression(paste("Neural dissimilarity (1- ",italic("r"), ")")))
...
Stress <- total %>% filter(Stress.x == "Stress") %>% 
  dplyr::select(Trans_X,Trans_Y,Trans_Z,Rot_Pitch,Rot_Roll,Rot_Yaw,Trans_X_b1,
                Trans_Y_b1,Trans_Z_b1,Rot_Pitch_b1,Rot_Roll_b1,Rot_Yaw_b1,
                Trans_X_b2,Trans_Y_b2,Trans_Z_b2,Rot_Pitch_b2,Rot_Roll_b2,
                Rot_Yaw_b2,Trans_X_b3,Trans_Y_b3,Trans_Z_b3,Rot_Pitch_b3,
                Rot_Roll_b3,Rot_Yaw_b3)


Control  <- total %>% filter(Stress.x == "Control") %>% 
  dplyr::select(Trans_X,Trans_Y,Trans_Z,Rot_Pitch,Rot_Roll,Rot_Yaw,Trans_X_b1,
                Trans_Y_b1,Trans_Z_b1,Rot_Pitch_b1,Rot_Roll_b1,Rot_Yaw_b1,
                Trans_X_b2,Trans_Y_b2,Trans_Z_b2,Rot_Pitch_b2,Rot_Roll_b2,
                Rot_Yaw_b2,Trans_X_b3,Trans_Y_b3,Trans_Z_b3,Rot_Pitch_b3,
...
G = "Control"
V = "Rot_Yaw_b1"
total[total$Stress.x== G,][which(total[total$Stress.x== G, V] > (mean(total[total$Stress.x== G, V], na.rm = T) + (2.5*SD(total[total$Stress.x== G, V])))),]
total[total$Stress.x== G,][which(total[total$Stress.x== G, V] < (mean(total[total$Stress.x== G, V], na.rm = T) - (2.5*SD(total[total$Stress.x== G, V])))),]
...
total$Rot_Yaw[5] # --> VP 38 is anyway outlier in RSA analysis
...
PHC <- as.data.frame(total[,c(1,2,4,5,6,110,111)])
PHC_long <- melt(PHC, id.vars = c("VP.x","Code","Stress.x","Sex.x","Age.x"
),measure = names(PHC[c(6,7)]))

for (i in 1:(dim(PHC_long)[1])) {
  if (substr(PHC_long$variable[i],1,2) == "Pr") {
    PHC_long$Time[i] <- "Pre"
  } else if (substr(PHC_long$variable[i],1,2) == "Po"){
    PHC_long$Time[i] <- "Post"}
}


for (i in 1:(dim(PHC_long)[1])) {
  if (substr(PHC_long$variable[i],1,2) == "Pr" & PHC_long$Stress.x[i] == "Control") {
    PHC_long$Group[i] <- "Con Pre"
    PHC_long$Order[i] <- "pre"
  } else if (substr(PHC_long$variable[i],1,2) == "Pr" & PHC_long$Stress.x[i] == "Stress") {
    PHC_long$Group[i] <- "Stress Pre"
    PHC_long$Order[i] <- "post"
  } else if (substr(PHC_long$variable[i],1,2) == "Po" & PHC_long$Stress.x[i] == "Control") {
    PHC_long$Group[i] <- "Con Post"
    PHC_long$Order[i] <- "pre"
  } else if (substr(PHC_long$variable[i],1,2) == "Po" & PHC_long$Stress.x[i] == "Stress") {
    PHC_long$Group[i] <- "Stress Post"
    PHC_long$Order[i] <- "post"
  }
}
...
t.test(PHC_long[which(PHC_long$Time == "Pre" & PHC_long$Stress.x == "Control"),]$value,
       PHC_long[which(PHC_long$Time == "Post" & PHC_long$Stress.x == "Control"),]$value, paired = T)


t.test(PHC_long[which(PHC_long$Time == "Pre" & PHC_long$Stress.x == "Stress"),]$value,
       PHC_long[which(PHC_long$Time == "Post" & PHC_long$Stress.x == "Stress"),]$value, paired = T)
...
p <-ggplot(PHC_long, aes(x = Stress.x, y = value, fill = Group))+
  ...
  scale_colour_manual("Group",values = c("Con Pre"="darkslategray3", "Con Post" = "darkslategray4", 
                                         "Stress Pre" = "brown1", "Stress Post" = "brown4")) +
  ylab('Putamen activity') + xlab ('') + stat_summary(fun.data = mean_se, size = 1, geom = "errorbar",  position=position_dodge(0.6),width =.1)+
  apatheme

p + coord_cartesian(ylim = c(-2, 2.5))+
  annotate(geom="text", x=2, y=2.3, label= c("* * *"), color="black", fontface = "bold", size = 6) +
  geom_segment(aes(x=1.85,xend=2.15,y=2.1,yend=2.1))+
  annotate(geom="text", x=1, y=2.3, label= c("* *"), color="black", fontface = "bold", size = 6) +
  geom_segment(aes(x=.85,xend=1.15,y=2.1,yend=2.1))


#----------------------------------------------------------------------------------

# Free recall: look at details for link event 
for (i in 1:(dim(total)[1])) {
  if (substr(total$Counterbalancing[i],1,2) == "L1") {
    total$Link_Details[i] <- total$FreeRecall_L1_total[i]
  } else if (substr(total$Counterbalancing[i],1,2) == "L2"){
    total$Link_Details[i] <- total$FreeRecall_L2_total[i]
  } }
...
t.test(total[which(total$Stress.x == "Stress"),]$Link_Details,
       total[which(total$Stress.x == "Control"),]$Link_Details, paired = F)
...
p <-ggplot(total, aes(x = Stress.x, y = Link_Details, fill = Stress.x))+
  stat_summary(aes(x = x),fun='mean',geom='bar', position=position_dodge(0.9), width = 0.4, alpha = 0.6)+
  geom_point(aes(x = x, colour=Stress.x, fill = Stress.x),pch = 21, size = 3, position = position_jitterdodge(jitter.width = 0.05, dodge.width = 0.4), alpha = 0.6)+
  scale_fill_manual("Stress.x",values = c("Control" = "darkslategray4","Stress"="brown1")) + 
  scale_colour_manual(values=c("darkslategray4", "brown1"))+ xlab('')+
  apatheme + ylab('Details') + stat_summary(aes(x = x),fun.data = mean_se, size = 1, geom = "errorbar",  position=position_dodge(0.9),width =.05)+
  scale_x_continuous(expand=expansion(mult=c(0.2,1)), breaks = c(1,1.5,2,2.5), labels = c('Control','Stress','',''))

p+coord_cartesian(ylim = c(0, 29)) 


#------
# Free recall: split up by events (A, B, and X)
Free <- as.data.frame(my_data[,c(1,2,99,4,5,6,56, 57, 58)])
Free <- Free[c(-4, -16, -28, -39, -46),]
...
p <-ggplot(Free_long, aes(x = Item, y = Details, fill = Stress))+
  stat_summary(stat = 'identity', fun='mean',geom='bar', position=position_dodge(0.6), size = 1.5,width = 0.6, alpha = 0.6) +
  geom_point(aes(colour=Stress),pch = 19, size = 3, position = position_dodge(0.6), # new version (no more jittering)
             alpha = 0.4)+
  #geom_line(aes(x = x, group = interaction(Code, Stress), color = Group), alpha = 0.2) +
  scale_fill_manual("Stress",values = c("Control"="darkslategray4", "Stress" = "brown1")) +
  scale_colour_manual("Stress", values = c("Control"="darkslategray4", "Stress" = "brown1")) + 
...
p+coord_cartesian(ylim = c(0, 29))  +
  annotate(geom="text", x=2, y=29, label= c("* *  -  * * *"), color="black", fontface = "bold", size = 7) +
  geom_segment(aes(x=1,xend=3,y=28,yend=28))+
  annotate(geom="text", x=2.5, y=25, label= c("*  -  * * *"), color="black", fontface = "bold", size = 7) +
  geom_segment(aes(x=2,xend=3,y=24,yend=24))+ theme(legend.position = "none")


#---------------------
# Correlation between details for items and link events
